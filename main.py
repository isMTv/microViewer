...
from cversion import set_components_version
...
class MvApp:
        ...
        if MV.connection_type == "id":
            if Check.id(tunnel_local_port, str(tunnel_remote_port)):
                ...
                ExternalApp.popen_hide(
                    f"{k_app} -P {srv_port} -i {key} -T -N -{mode} :{tunnel_local_port}:127.1:{tunnel_remote_port} {user}@{Tunnel.host} -auto-store-sshkey -send-to-tray",
                    run_type)
                sleep(3)
                return True
            else:
    ...
    def client_on_rd():
        global modes_client
        if MV.connection_type == "id":
            ExternalApp.popen(f"bin/uvnc/vncviewer.exe -connect 127.0.0.1:{connect_id} -password {connect_pw} -noauto -64colors -encoding zrle -disablesponsor -config bin/uvnc/viewer.vnc", "client")
        elif MV.connection_type == "ip":
            ExternalApp.popen(f"bin/uvnc/vncviewer.exe -connect {connect_id}:{RemoteDesktop.portnumber} -password {connect_pw} -noauto -64colors -encoding zrle -disablesponsor -config bin/uvnc/viewer.vnc", "client")
        modes_client = True
