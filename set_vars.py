...
k_app = "bin/kitty.exe"
k_service = Path(dir_path, "bin/kitty_service.exe")
k_task = Path(dir_path, "bin/tasks/k_tunnel.xml")
...
class Service:
    ...
    disable_sleep_mode: str = (get_config(mv_conf_path, "Service", "disable_sleep_mode")).lower()
    disable_hibernation_mode: str = (get_config(mv_conf_path, "Service", "disable_hibernation_mode")).lower()
    disable_lock_screen: str = (get_config(mv_conf_path, "Service", "disable_lock_screen")).lower()
    all: tuple = ("enabled", "fix_id_pw", "id", "password", "check_interval", "disable_sleep_mode",
                  "disable_hibernation_mode", "disable_lock_screen")
...
class Pids:
    tun_service: str = get_config(mv_conf_path, "Pids", "tun_service")
    all: tuple = ("tun_service",)
...
def refresh_classes_vars(conf_path, data_class):
...
def refresh_profiles(conf_path, data_class):
    for index in eval(data_class).all:
        data = get_config(conf_path, data_class, index)
        command = f'{data_class}.{index} = {data}'
        exec(command)
...
def refresh_uvnc(conf_path, data_class):
    for index in eval(data_class).all:
